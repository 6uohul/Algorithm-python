...
B 나라는 특정 x 좌표에서 y 축에 수평이 되도록 미사일을 발사하며, 발사된 미사일은 해당 x 좌표에 걸쳐있는 모든 폭격 미사일을 관통하여 
한 번에 요격할 수 있습니다. 단, 개구간 (s, e)로 표현되는 폭격 미사일은 s와 e에서 발사하는 요격 미사일로는 요격할 수 없습니다. 
...
def solution(targets):
    count = 0 # 처음에 커서를 첫번째 타겟의 끝에 두므로 한번 카운트 시작
    cur = 0
    targets.sort(key = lambda x: x[1])
    
    for start, end in targets:
        if start >= cur:  # 새로운 요격이 필요할 경우
            count += 1
            cur = end  # 커서를 현재 타겟의 끝으로 이동
